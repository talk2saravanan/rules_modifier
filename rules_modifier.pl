...
sub help {
	...
	e.g >perl rules_modifier.pl --rules_filename=database_server regex_filename=regex.txt
...
my $rules = read_contents($rules_file);
my  $regex = read_contents($regex_file);
if($rules && $regex ) {
	
	foreach my  $rule(@$rules) {
		next if ($rule=~/^#/s || $rule =~/^\s*$/s);
	...
	if(write_contents("$rules_file-bkp",$rules)) {
